...
async function safeDBRun(query, ...params) {
  ...
  catch (err) { console.error('💥 DB 실행 에러:', err, query, params); throw err; }
}
async function safeDBGet(query, ...params) {
  try { return await db.get(query, ...params); } 
  catch (err) { console.error('💥 DB 조회 에러:', err, query, params); throw err; }
}
async function safeDBAll(query, ...params) {
  try { return await db.all(query, ...params); } 
  catch (err) { console.error('💥 DB 전체 조회 에러:', err, query, params); throw err; }
}
...
async function findLotteryChannel(client) {
  // 봇이 속한 서버들 중 '복권' 또는 'lottery' 포함된 채널 우선 탐색
...
export async function execute(interaction) {
  try {
    ...
    console.error('❌ 수동 복권결과 명령어 에러:', err);
    await interaction.reply('⚠️ 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
  }
}
...
const horses = [
  { name:"썬더", emoji:"🐎" }, { name:"스피드", emoji:"🐎" },
  { name:"라이트닝", emoji:"🐎" }, { name:"블레이드", emoji:"🐎" },
  { name:"토네이도", emoji:"🐎" }, { name:"스타", emoji:"🐎" },
  { name:"썬샤인", emoji:"🐎" }
];
