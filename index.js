...
// ===== Express 서버 (Render용 Keep-Alive) =====
const app = express();
app.get('/', (_, res) => res.send('봇 정상 작동 중 ✅'));
app.listen(process.env.PORT || 10000, () => console.log('🌐 Express 서버 실행 완료'));
...
async function safeRun(query, ...params) {
  ...
  catch (err) { console.error('💥 DB Run Error:', err); }
}
async function safeGet(query, ...params) {
  try { return await db.get(query, ...params); }
  catch (err) { console.error('💥 DB Get Error:', err); }
}
async function safeAll(query, ...params) {
  try { return await db.all(query, ...params); }
  catch (err) { console.error('💥 DB All Error:', err); }
}

// ===== 유틸 =====
async function getUser(id) {
  let user = await safeGet('SELECT * FROM users WHERE id = ?', id);
  if (!user) {
    await safeRun('INSERT INTO users (id, balance) VALUES (?, ?)', id, 1000);
...
async function updateBalance(userId, amount, reason) {
  ...
  try {
    ...
    await safeRun('UPDATE users SET balance = ? WHERE id = ?', newBalance, userId);
    await safeRun(
...
const commands = [
  ...
  new SlashCommandBuilder().setName('골라')
    .setDescription('여러 옵션 중 하나를 무작위로 선택')
    .addStringOption(opt => opt.setName('옵션들').setDescription('쉼표로 구분').setRequired(true)),
  ...
  new SlashCommandBuilder().setName('복권구매')
    .setDescription('복권 구매')
    .addStringOption(opt => opt.setName('번호').setDescription('1~45 중 6개').setRequired(true)),
  new SlashCommandBuilder().setName('복권상태').setDescription('오늘의 복권 구매 상태'),
  new SlashCommandBuilder().setName('경마')
    .setDescription('랜덤 경마 게임')
    .addIntegerOption(opt => opt.setName('베팅').setDescription('베팅 금액').setRequired(true))
    .addIntegerOption(opt => opt.setName('말번호').setDescription('1~7 선택').setRequired(true)),
  new SlashCommandBuilder().setName('관리자지급')
    .setDescription('관리자 전용 포인트 지급')
    .addUserOption(opt => opt.setName('대상').setDescription('유저 선택').setRequired(true))
    .addIntegerOption(opt => opt.setName('금액').setDescription('지급 금액').setRequired(true)),
];
...
cron.schedule('0 21 * * *', async () => {
  try {
    ...
    const tickets = await safeAll('SELECT * FROM lottery_tickets WHERE draw_date = ?', today);
    ...
    for (const ticket of tickets) {
      ...
      if (matches >= 3) {
        ...
        await updateBalance(ticket.user_id, reward, `복권 ${matches}개 일치`);
      }
    }
  } catch (err) {
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    if (commandName === '돈줘') {
      ...
      await safeRun('UPDATE users SET last_claim=? WHERE id=?', now, user.id);
      const bal = await updateBalance(user.id, 1000, '기본금 지급');
      return interaction.reply(`💸 1000원 지급 완료. 현재 잔고: ${bal}원`);
    }

    if (commandName === '잔고')
      return interaction.reply(`💰 현재 잔고: ${userData.balance}원`);

    if (commandName === '골라') {
      const opts = options.getString('옵션들').split(',').map(o => o.trim()).filter(Boolean);
    ...
    if (commandName === '관리자지급') {
      ...
      return interaction.reply(`✅ ${target.username}에게 ${amt}원 지급 완료. (잔고: ${newBal})`);
    }
  } catch (err) {
    console.error('💥 Command Error:', err);
    return interaction.reply('⚠️ 명령 실행 중 오류 발생.');
  }
});
