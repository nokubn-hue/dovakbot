...
async function getUser(userId) {
  ...
  if (!row) {
    ...
    return { id: userId, balance: 0, last_claim: 0 };
  }
...
async function startRace(channel,bettors){
  ...
  return new Promise(resolve=>{
    ...
    const interval=setInterval(async()=>{
      ...
      const raceMsg=positions.map((p,i)=>`${horses[i].emoji} ${horses[i].name.padEnd(8," ")} |${"·".repeat(p)}${" ".repeat(Math.max(0,trackLength-p))}🏁`).join("\n");
    ...
    setTimeout(()=>{
      if(!finished){ clearInterval(interval); msg.reply("⏱ 경주 시간초과 종료"); resolve(null); }
    },40000);
  });
}
...
const commandList=[
  ...
  new SlashCommandBuilder().setName("골라").setDescription("옵션 중 무작위 선택").addStringOption(o=>o.setName("option").setDescription("예: a,b,c").setRequired(true)).addIntegerOption(o=>o.setName("count").setDescription("개수").setRequired(false)),
  ...
  new SlashCommandBuilder().setName("경마").setDescription("경마 게임").addIntegerOption(o=>o.setName("번호").setDescription("1~7 선택").setRequired(true)).addIntegerOption(o=>o.setName("배팅").setDescription("배팅 금액")),
...
client.on("interactionCreate",async(interaction)=>{
  ...
  try{
    ...
    if(cmd==="돈줘"){
      ...
      await changeBalance(uid,DEFAULT_STARTING,"daily_claim");
      await db.run("UPDATE users SET last_claim=? WHERE id=?",now,uid);
    ...
    if(cmd==="골라"){
      ...
      const picks=pickRandomOption(raw,count);
      if(!picks){ await interaction.reply("유효한 옵션이 없습니다."); return; }
      const content=count===1?`✅ 선택: **${picks[0]}**`:`✅ ${count}개 선택: ${picks.map(p=>`**${p}**`).join(", ")}`;
    ...
    if(cmd==="관리자지급"){
      ...
      await interaction.reply(`<@${target.id}>에게 ${amount}포인트 적용 완료`);
      return;
    }
    ...
    if(cmd==="블랙잭"){
      ...
      if(bet<=0){ await interaction.editReply("배팅 금액은 1 이상이어야 합니다."); return; }
      const user=await getUser(uid);
      ...
      if(playerTotal>21) resultText+=`❌ 버스트! 패배\n💸 배팅액 ${bet}포인트 차감`;
      else if(dealerTotal>21 || playerTotal>dealerTotal){ const payout=bet*2; await changeBalance(uid,payout,"blackjack_win"); resultText+=`✅ 승리! 💰 ${payout}포인트 지급`; }
      else if(playerTotal===dealerTotal){ await changeBalance(uid,bet,"blackjack_draw"); resultText+=`🤝 무승부! 배팅액 ${bet} 반환`; }
      else resultText+=`❌ 딜러 승리! 💸 배팅액 ${bet} 차감`;
      resultText+=`\n현재 잔고: ${(await getUser(uid)).balance}포인트`;
    ...
    if(cmd==="바카라"){
      ...
      const bet=Number(interaction.options.getInteger("배팅")||100);
      if(!["플레이어","뱅커","무승부"].includes(side)){ await interaction.editReply("배팅방향 오류"); return; }
      if(bet<=0){ await interaction.editReply("배팅 1 이상"); return; }
      const user=await getUser(uid);
      if(user.balance<bet){ await interaction.editReply("잔고 부족"); return; }
      await changeBalance(uid,-bet,"baccarat_bet");
      const deck=createDeck(); const draw=()=>deck.pop();
      const playerCards=[draw(),draw()]; const bankerCards=[draw(),draw()] ;
      const baccaratValue=c=>["J","Q","K","10"].includes(c)?0:c==="A"?1:Number(c);
      const total=a=>a.reduce((s,c)=>s+baccaratValue(c),0)%10;
      const playerTotal=total(playerCards); const bankerTotal=total(bankerCards);
      let winner=playerTotal>bankerTotal?"플레이어":bankerTotal>playerTotal?"뱅커":"무승부";
      let payout=0; let text=`🎴 바카라 결과 🎴\n플레이어: ${playerCards.join(", ")} (${playerTotal})\n뱅커: ${bankerCards.join(", ")} (${bankerTotal})\n--------------------------\n`;
      if(side===winner){
        if(winner==="플레이어") payout=bet*2;
        else if(winner==="뱅커") payout=Math.floor(bet*1.95);
        else payout=bet*9;
        await changeBalance(uid,payout,"baccarat_win");
        text+=`✅ 선택한 ${side} 승리! 💰 상금 ${payout} 지급`;
      } else text+=`❌ 선택한 ${side} 패배 💸 배팅액 ${bet} 차감`;
      text+=`\n현재 잔고: ${(await getUser(uid)).balance}포인트`;
    ...
    if(cmd==="경마"){
      await interaction.deferReply();
      const num=Number(interaction.options.getInteger("번호")||1)-1;
      const bet=Number(interaction.options.getInteger("배팅")||100);
      if(num<0 || num>=horses.length){ await interaction.editReply("번호 1~7 선택"); return; }
      const user=await getUser(uid);
      if(user.balance<bet){ await interaction.editReply("잔고 부족"); return; }
      await changeBalance(uid,-bet,"race_bet");
      const bettors=new Map(); bettors.set(uid,{horseIndex:num,bet});
      await startRace(interaction.channel,bettors);
      await interaction.editReply("경마 결과 완료!");
      return;
    }

  }catch(e){ console.error("interaction 오류:",e); }
});
